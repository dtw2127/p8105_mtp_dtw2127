...
We examine the findings presented in the 2018 article on the
relationship between posture and enlarged external occipital
protuberances. We examine data consistency, results presentation and try
...
Key variables in the dataset include sex and age, eop\_size,
eop\_visibility\_classification, eop\_shape and fhp\_size. There are
1219 participants included in the dataset.
...
The 18-30, and 60+ years old are the largest groups. There are roughly
...
0 participants had incorrect EOP size categorizations. Many participants
were assigned EOP size 4 instead of 5.
...
0 participants had their visibility classification incorrectly assigned.
Many participants were categorized as EOP size 0 instead of 1.
...
27 individuals had an incorrect FHP categorization (FHP category
underestimated).
...
From the right panel, We can see a higher rate of EEOP in males. The
highest rates of EEOP are in the youngest age group. If FHP is
predictive of EEOP, we would expect the FHP size trend on the left to be
similar to that of the EEOP rate trend, but this is not the case. While
FHP sizes appear to be larger in males as compared to females, it isn’t
clear if the differences are significant.
...
There is no association between EOP size and FHP size in any of the age
...
Sample sizes calculated are similar to those reported by the authors
...
The prevalence we calculate is 0.3223954 is similar to the authors’
calculations, but \~1% off.
...
Our prevalence calculation for 60+ years is different by \~2%.
...
Based on this paper, we cannot conclude that cell phones are causing
horn growth. We would need to see data on cell phone usage, and evidence
that screen-based activity impacts FHP size.
